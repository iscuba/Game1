...
//Both add functions add at the front of the arrayList 
    ...
    public boolean offGridHuh() {
        int num = movingBlocks.size() - 1;
        return (movingBlocks.get(num).x <= 1) || (movingBlocks.get(0).x >= 11);
    }
...
//        public WorldImage = new OverlayImage()
        public WorldImage makeImage(){
            for (int i =0; i<movingBlocks.size();i++){
                OverlayImages(backdrop,movingBlocks.get(i).blockImage());
            }
        }
